...
let offSetRect = 10
let svgOffSetTop = 100 //the top of the svg where the graph is drawn
...
function circleElement(id) {
	let elem = document.getElementById(id)
	return elem && elem.getElementsByTagName('circle')[0]
}
...
function createNode(id, x, y, w, h, label, c) {
	...
	if (c == 'undefined') { circle.setAttribute('fill', 'None') } else { circle.setAttribute('fill', c) }
...
function updateNode(id, x, y, w, h, label, c) {
	...
	if (oldNodeIds.indexOf(id) < 0) {
		...
		//Animate circle to new position
		let circle = circleElement(id)
		if (c == 'undefined') { circle.setAttribute('fill', 'None') } else { circle.setAttribute('fill', c) }
		anime({
			targets: circle,
			cx: dpi(x),
...
function parseGraph(s) {
	...
	s.split('\n').forEach(l => {
		...
		switch(line[0]) {
			...
			case 'node':
				...
				clusters[c].push(id)
				//update the node with the given parameters
				updateNode(id, x, y, w, h, label, c)
	...
	for (let c in clusters) {
		...
		if (doc == null) {
			doc = document.createElementNS('http://www.w3.org/2000/svg', 'g')
			doc.setAttribute("id", c)
		}
		...
		if (rec == null) {
			...
			rec.setAttribute('id', "rect_"+c)
			rec.setAttribute('stroke', c)
			rec.setAttribute('fill', "none")
			rec.setAttribute('stroke-width', 6)
		}
		let pos = doc.getBoundingClientRect()
		anime({
			targets: rec,
			x: pos.x,
			y: pos.y,
