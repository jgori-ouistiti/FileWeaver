...
def check_link(file, send=True):
	...
	cakes_need_update, deps_need_update = need_update(g, v, FILE_ACCESS_TIME)

	if cakes_need_update:
	...
	cakes = (cakes, 1) if cakes_need_update else (cakes, 0)
	deps = (deps, 1) if deps_need_update else (deps, 0)
...
def most_recent_update(edges):
	#edges[:, 2] is the time it has been last touched (UNIX time)
	#edges[:, 3] is bool if the edge has been updated or not
	return numpy.min(numpy.ma.masked_array(edges[:, 2], ~edges[:, 3].astype(bool)))
